    ...
    <div className={cn(
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center gap-3">
          <div className={cn(
            ...
            isUrgent ? "bg-danger text-white" : isWarning ? "bg-warning text-background" : "bg-primary text-background"
          )}>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/5">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold transition-all group-hover:bg-primary group-hover:text-background shadow-lg shadow-black/20 disabled:opacity-40"
        >
          <Bell className="w-4 h-4" />
          {locales.insurance.actions.notify}
        </button>
      </div>
    </div>
  )
